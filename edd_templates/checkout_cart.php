...
<table id="edd_checkout_cart" <?php if ( ! edd_is_ajax_disabled() ) {
			...
			<tr class="edd_cart_item" id="edd_cart_item_<?php echo esc_attr( $key ) . '_' . esc_attr( $item['id'] ); ?>"
				...
				<td class="edd_cart_item_name">
					...
					if ( current_theme_supports( 'post-thumbnails' ) && has_post_thumbnail( $item['id'] ) ) {
						...
						echo get_the_post_thumbnail( $item['id'], apply_filters( 'edd_checkout_image_size', array(
								25,
								25
						) ) );
						echo '</div>';
				...
				<td class="edd_cart_actions">
					...
					<a class="edd_cart_remove_item_btn"
					   href="<?php echo esc_url( wp_nonce_url( edd_remove_item_url( $key ), 'edd-remove-from-cart-' . $key, 'edd_remove_from_cart_nonce' ) ); ?>"><span
	...
	<?php if ( edd_use_taxes() && ! edd_prices_include_tax() ) : ?>
		<tr class="edd_cart_footer_row edd_cart_subtotal_row"<?php if ( ! edd_is_cart_taxed() ) {
	...
	<tr class="edd_cart_footer_row edd_cart_discount_row" <?php if ( ! edd_cart_has_discounts() ) {
		echo ' style="display:none;"';
	} ?>>
	...
	<?php if ( edd_use_taxes() ) : ?>
		<tr class="edd_cart_footer_row edd_cart_tax_row"<?php if ( ! edd_is_cart_taxed() ) {
			echo ' style="display:none;"';
		} ?>>
	...
	<tr class="edd_cart_footer_row edd_cart_footer_row_total">
		...
		<th class="edd_cart_total edd_cart_total_text">
			<span><?php _e( 'Your total today', 'easy-digital-downloads' ); ?>: </span></th>
		<th class="edd_cart_total"><span class="edd_cart_amount" data-subtotal="<?php echo edd_get_cart_subtotal(); ?>"
